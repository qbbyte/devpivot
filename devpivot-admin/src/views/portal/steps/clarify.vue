<template>
  <div class="clarify-page">
                ...
                <div class="message-content">
                  ...
                  <div class="message-options" v-if="msg.options">
                    <div
                      ...
                      :key="opt.value"
                      class="option-item"
                      :class="{ selected: msg.selectedOption === opt.value }"
    ...
    >
      <div class="model-dialog-body">
        ...
        <div class="model-list">
          ...
          >
            <el-checkbox
              :model-value="tempSelectedIds.includes(model.id)"
              :disabled="!tempSelectedIds.includes(model.id) && tempSelectedIds.length >= maxCompareCount"
              @click.stop
    ...
    <el-drawer
      v-model="responseDetail.visible"
    ...
    >
      ...
      <button
        v-show="retainBtn.visible"
        class="retain-floating"
        :style="{ top: retainBtn.y + 'px', left: retainBtn.x + 'px' }"
        @mousedown.prevent
        @click="confirmRetain"
      >保留</button>
    </el-drawer>
    ...
    <el-drawer
      v-model="showConclusion"
      title="需求澄清结论"
      direction="rtl"
      size="520px"
      :with-header="true"
      class="conclusion-drawer-root"
      @opened="onConclusionOpened"
    >
      <div class="conclusion-drawer">
        ...
        <section class="conclusion-section" v-if="clarifyConclusion.adopted.length">
          ...
          <div
            v-for="(a, i) in clarifyConclusion.adopted"
            :key="i"
            class="conclusion-adopt"
          >
        ...
        <section class="conclusion-section">
          <h4 class="conclusion-h" v-if="clarifyConclusion.freeInputs.length">用户补充（{{ clarifyConclusion.freeInputs.length }}）</h4>
        ...
        <section class="conclusion-section" v-if="clarifyConclusion.openQuestions.length">
          ...
          <div
            v-for="(o, i) in clarifyConclusion.openQuestions"
            :key="i"
            class="conclusion-open"
          >{{ o }}</div>
        </section>
        ...
        <div class="conclusion-supplement">
          <div class="conclusion-supp-head">
            <el-icon class="conclusion-supp-icon"><EditPen /></el-icon>
            <span class="conclusion-supp-label">补充说明（提交前可追加）</span>
            <span class="conclusion-supp-tag">可选</span>
          </div>
          <el-input
            v-model="conclusionSupplement"
            type="textarea"
            :autosize="{ minRows: 3, maxRows: 6 }"
            resize="none"
            placeholder="可在此补充需求背景、目标、约束或其他说明，将一并纳入澄清结论"
          />
        </div>
        </div>
...
.back-btn:hover { background: rgba(51, 112, 255, 0.08); color: var(--primary); }
.header-titles { display: flex; align-items: center; gap: 12px; min-width: 0; }
.header-title {
...
.stage-dot { width: 6px; height: 6px; border-radius: 50%; background: var(--primary); }
.history-btn {
...
.history-btn:hover { color: var(--primary); border-color: var(--primary); }
...
.panel-block + .panel-block { border-top: 1px solid var(--border); }
.block-label {
  display: block; font-size: 12px; color: var(--text-3); letter-spacing: 0.2px;
}
.block-value {
...
.chip-empty { display: block; margin-top: 9px; font-size: 13px; color: var(--text-4); }
.link-btn {
  margin-top: 12px; padding: 0; border: none; background: none;
  color: var(--primary); font-size: 13px; cursor: pointer; transition: color 0.18s ease;
}
.link-btn:hover { color: var(--primary-hover); }
.progress-row {
  display: flex; align-items: baseline; justify-content: space-between;
}
.progress-count { font-size: 16px; font-weight: 700; color: var(--text-1); }
.progress-count i { font-style: normal; font-size: 12px; font-weight: 400; color: var(--text-4); margin-left: 1px; }
.progress-track {
  margin-top: 11px; height: 6px; border-radius: 999px;
...
.multi-responses::-webkit-scrollbar { width: 8px; height: 8px; }
.chat-scroll::-webkit-scrollbar-thumb,
.multi-responses::-webkit-scrollbar-thumb { background: #dfe3e8; border-radius: 999px; }
.chat-scroll::-webkit-scrollbar-thumb:hover,
.multi-responses::-webkit-scrollbar-thumb:hover { background: #c6ccd4; }

/* welcome */
.chat-welcome {
  height: 100%;
  display: flex; flex-direction: column; align-items: center; justify-content: center;
  text-align: center; gap: 6px; color: var(--text-3);
}
.welcome-icon {
...
.welcome-title { font-size: 17px; font-weight: 600; color: var(--text-1); }
.welcome-desc { font-size: 13px; color: var(--text-3); max-width: 320px; line-height: 1.6; }

/* messages */
.chat-message { display: flex; gap: 12px; margin-bottom: 22px; animation: msg-in 0.32s ease both; }
...
.user-avatar { background: linear-gradient(135deg, #6b7785, #8a96a3); }
...
.ai-message .message-bubble { background: var(--surface); color: var(--text-1); border-top-left-radius: 5px; }
.user-message .message-bubble { background: linear-gradient(135deg, #3370ff, #4d82ff); color: #fff; border-top-right-radius: 5px; }
.adopt-bubble { background: #f0f9eb !important; color: #52a824 !important; display: inline-flex; align-items: center; gap: 6px; box-shadow: none !important; }
.message-options { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 12px; }
.option-item {
  padding: 7px 16px; border: 1px solid #e4e7ed; border-radius: 999px;
  font-size: 13px; color: var(--text-2); cursor: pointer; transition: all 0.18s ease;
}
...
.dot { width: 7px; height: 7px; background: var(--text-4); border-radius: 50%; animation: typing 1.4s infinite; }
...
.multi-fade {
  position: absolute; top: 0; right: 0; bottom: 6px; width: 64px;
...
.multi-arrow {
  ...
  background: var(--surface); color: var(--text-2);
  box-shadow: 0 2px 8px rgba(20, 23, 28, 0.12); cursor: pointer;
  transition: all 0.18s ease;
}
.multi-arrow:hover { color: var(--primary); border-color: var(--primary); box-shadow: 0 3px 12px rgba(51, 112, 255, 0.2); }
.multi-arrow:active { transform: translateY(-50%) scale(0.92); }
.response-card {
  min-width: 300px; flex: 1 0 300px;
...
.resp-body.is-overflow .resp-fade { display: block; }
.resp-body.is-overflow .view-more { display: inline-flex; }
.resp-fade {
  ...
  pointer-events: none; border-bottom-left-radius: var(--radius-md); border-bottom-right-radius: var(--radius-md);
}
.view-more {
  display: none; position: absolute; right: 10px; bottom: 8px; align-items: center; gap: 2px;
  padding: 4px 10px; font-size: 12px; color: var(--primary);
  background: var(--surface); border: 1px solid var(--primary); border-radius: 14px;
  cursor: pointer; box-shadow: 0 2px 6px rgba(51, 112, 255, 0.15); transition: all 0.18s ease;
}
.view-more:hover { background: var(--primary); color: #fff; }
.response-detail { height: 100%; overflow-y: auto; }
.response-detail-content { font-size: 14px; line-height: 1.7; color: var(--text-1); padding-right: 4px; }

/* 抽屉：已保留要点 */
.retain-drawer { display: flex; flex-direction: column; gap: 10px; }
.retain-hint { font-size: 13px; color: var(--text-4); line-height: 1.6; padding: 20px 4px; }
.retain-item { border: 1px solid var(--border); border-radius: var(--radius-md); padding: 12px 14px; background: #fafbfc; }
.retain-item-head { display: flex; align-items: center; gap: 8px; margin-bottom: 6px; }
.retain-model { font-size: 12px; font-weight: 600; color: var(--primary); }
.retain-time { font-size: 11px; color: var(--text-4); flex: 1; }
.retain-del { border: none; background: none; color: var(--text-3); cursor: pointer; font-size: 16px; line-height: 1; padding: 0 2px; margin-left: auto; }
.retain-del:hover { color: #f53f3f; }
.retain-text { font-size: 13px; line-height: 1.6; color: var(--text-1); white-space: pre-wrap; word-break: break-word; }
.retain-fill { margin-top: 16px; width: 100%; font-size: 13px; font-weight: 500; color: var(--primary); background: var(--surface); border: 1px solid var(--primary); border-radius: var(--radius-md); padding: 9px 0; cursor: pointer; transition: all 0.18s ease; }
.retain-fill:hover { background: var(--primary); color: #fff; }

/* 抽屉内选区保留浮动按钮 */
.retain-floating { position: fixed; transform: translate(-50%, 0); z-index: 3000; padding: 5px 14px; font-size: 12px; color: #fff; background: var(--primary); border: none; border-radius: 14px; cursor: pointer; box-shadow: 0 4px 14px rgba(51, 112, 255, 0.35); }
.retain-floating:hover { background: #245bdb; }
...
.conclusion-summary {
  ...
  background: rgba(51, 112, 255, 0.06); border: 1px solid rgba(51, 112, 255, 0.18);
  border-radius: var(--radius-md); padding: 12px 14px;
}
.conclusion-section { }
.conclusion-h { font-size: 14px; font-weight: 600; color: var(--text-1); margin: 0 0 10px; padding-left: 9px; border-left: 3px solid var(--primary); }
.conclusion-h.open { border-left-color: #f59e0b; }
.conclusion-adopt, .conclusion-snippet {
  border: 1px solid var(--border); border-radius: var(--radius-md);
  padding: 12px 14px; margin-bottom: 10px; background: var(--surface);
}
.conclusion-meta { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; margin-bottom: 8px; }
.conclusion-model { font-size: 12px; font-weight: 600; color: var(--primary); }
.conclusion-time { font-size: 12px; color: var(--text-4); }
.conclusion-q { font-size: 12px; color: var(--text-3); flex: 1; min-width: 0; }
.conclusion-content { font-size: 13px; line-height: 1.7; color: var(--text-1); word-break: break-word; }
.conclusion-content :deep(strong) { color: var(--text-0); }
.conclusion-open { font-size: 13px; line-height: 1.7; color: #b45309; background: rgba(245, 158, 11, 0.08); border-radius: var(--radius-sm); padding: 8px 12px; margin-bottom: 8px; }
.conclusion-models { font-size: 13px; color: var(--text-1); }
.response-actions { display: flex; justify-content: flex-end; margin-top: auto; }
.multi-empty {
  padding: 22px; text-align: center; font-size: 13px; color: var(--text-4);
  background: #fafbfc; border: 1px dashed var(--border); border-radius: var(--radius-md);
}
.multi-hint {
...
.input-box {
  border: 1px solid #e6e8eb; border-radius: var(--radius-md);
  background: #f7f8fa; transition: all 0.2s ease;
}
.input-box.active { border-color: var(--primary); background: var(--surface); box-shadow: 0 0 0 3px rgba(51, 112, 255, 0.08); }
.input-box :deep(.el-textarea__inner) {
  box-shadow: none; background: transparent; border: none;
  padding: 12px 16px 2px; font-size: 14px; line-height: 1.6; resize: none;
}
.input-box :deep(.el-textarea__inner:focus) { box-shadow: none; }
.input-actions { display: flex; align-items: center; justify-content: space-between; padding: 2px 10px 8px 16px; }
.input-tip { font-size: 12px; color: var(--text-4); }
.send-btn { width: 34px; height: 34px; box-shadow: var(--shadow-sm); }
.send-loading { width: 14px; height: 14px; border: 2px solid #fff; border-top-color: transparent; border-radius: 50%; animation: spin 0.8s linear infinite; }
...
.model-dialog-tip { margin: 0 0 16px; font-size: 13px; color: var(--text-3); }
...
.model-item.selected { border-color: var(--primary); background: var(--primary-soft); }
.model-item.disabled { opacity: 0.45; cursor: not-allowed; }
.model-info { flex: 1; display: flex; flex-direction: column; gap: 3px; min-width: 0; }
.model-info .model-name { font-size: 14px; font-weight: 500; color: var(--text-1); }
.model-info .model-desc { font-size: 12px; color: var(--text-3); }
.conclusion-supplement { margin-top: 14px; padding: 14px 14px 16px; background: var(--primary-soft); border: 1px solid rgba(51, 112, 255, 0.22); border-radius: var(--radius-md); }
.conclusion-supp-head { display: flex; align-items: center; gap: 6px; margin-bottom: 10px; }
.conclusion-supp-icon { color: var(--primary); font-size: 15px; }
.conclusion-supp-label { font-size: 13px; font-weight: 600; color: var(--text-1); }
.conclusion-supp-tag { margin-left: auto; font-size: 11px; color: var(--primary); background: var(--surface); border: 1px solid rgba(51, 112, 255, 0.25); border-radius: 10px; padding: 1px 8px; }
.conclusion-supplement :deep(.el-textarea__inner) { background: var(--surface); border-color: rgba(51, 112, 255, 0.3); box-shadow: none; }
.model-dialog-footer { display: flex; align-items: center; justify-content: space-between; margin-top: 14px; padding-top: 12px; border-top: 1px solid var(--border); }
.selected-count { font-size: 13px; color: var(--text-2); }
...
.conclusion-hint .el-icon { color: #e6a23c; font-size: 14px; }

/* ---------- Responsive ---------- */
@media (max-width: 900px) {
  .layout { grid-template-columns: 1fr; overflow-y: auto; }
  .panel { align-self: stretch; }
  .multi-responses { flex-direction: column; }
  .response-card { min-width: 100%; flex-basis: auto; }
  .stage-pill { display: none; }
}

/* ---------- History Drawer ---------- */
.history-drawer { display: flex; flex-direction: column; gap: 12px; }
.history-item {
  background: var(--surface);
...
.history-item:hover { border-color: #d9dde3; }
.history-head { padding: 14px 16px; cursor: pointer; }
.history-head-top { display: flex; align-items: center; gap: 8px; }
.history-version { font-size: 14px; font-weight: 600; color: var(--text-1); }
.history-status {
  font-size: 11px; padding: 1px 8px; border-radius: 999px; font-weight: 500;
}
.history-status.is-draft { background: var(--primary-soft); color: var(--primary); }
.history-status.is-archived { background: #f2f3f5; color: var(--text-3); }
.history-caret { margin-left: auto; color: var(--text-4); transition: transform 0.2s ease; }
.history-caret.open { transform: rotate(180deg); }
.history-sub { display: flex; align-items: center; gap: 6px; margin-top: 6px; font-size: 12px; color: var(--text-3); }
.history-sep { color: var(--text-4); }
.history-summary { margin-top: 8px; font-size: 13px; line-height: 1.5; color: var(--text-2); }
.history-file-count { margin-top: 10px; font-size: 12px; color: var(--text-4); }
.history-files { border-top: 1px solid var(--border); background: #fafbfc; padding: 6px 16px 10px; }
.file-row {
...
.file-badge.ft-pdf { background: #fdeeee; color: #f5654a; }
.file-badge.ft-doc { background: #eaf2fd; color: #2f7de0; }
.file-badge.ft-xls { background: #eaf7ee; color: #1fa463; }
.file-badge.ft-dia { background: #f3edfd; color: #8b5cf6; }
.file-name { flex: 1; min-width: 0; font-size: 13px; color: var(--text-1); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.file-size { font-size: 12px; color: var(--text-4); flex-shrink: 0; }
.file-view {
  flex-shrink: 0; border: none; background: none; cursor: pointer;
  color: var(--primary); font-size: 13px; padding: 4px 8px; border-radius: 6px;
  transition: background 0.18s ease;
}
.file-view:hover { background: var(--primary-soft); }
</style>
