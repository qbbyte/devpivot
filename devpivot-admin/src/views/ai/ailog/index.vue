<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="场景类型" prop="taskType">
        <el-select v-model="queryParams.taskType" placeholder="请选择场景类型" clearable>
          <el-option
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:ailog:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['system:ailog:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="ailogList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['system:ailog:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['system:ailog:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="ailogRef" :model="form" :rules="rules" label-width="100px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="项目ID" prop="projectId">
          ...
          <el-col :span="24">
            <el-form-item label="场景类型" prop="taskType">
              <el-select v-model="form.taskType" placeholder="请选择场景类型">
                <el-option
                  v-for="dict in ai_scene_type"
                  :key="dict.value"
                  :label="dict.label"
...
import { listAilog, getAilog, delAilog, addAilog, updateAilog } from "@/api/ai/ailog"

const { proxy } = getCurrentInstance()
const { ai_call_status, sys_yes_no, ai_scene_type } = useDict('ai_call_status', 'sys_yes_no', 'ai_scene_type')
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["ailogRef"].validate(valid => {
    if (valid) {
      if (form.value.logId != null) {
        updateAilog(form.value).then(() => {
          proxy.$modal.msgSuccess("修改成功")
          open.value = false
          getList()
        })
      } else {
        addAilog(form.value).then(() => {
...
function handleExport() {
  proxy.download('system/ailog/export', {
    ...queryParams.value
